...
--计算每个检验值每天多少人就可以看出是缺失值多少
WITH labevents2 AS(
SELECT labevents1.dlabel,
       COUNT(*) AS numbers
FROM labevents1
GROUP BY labevents1.dlabel
ORDER BY numbers DESC
)
SELECT labevents2.*
FROM labevents2
WHERE labevents2.numbers > 3553
;



---筛查后长转宽纳入所有可行的实验室检查,MAX还会导致没有最大值的subject缺失，所以行数会下降
...
---药物筛查，每个人每种药物
DROP TABLE IF EXISTS drugs1;
CREATE TEMP TABLE drugs1 AS
SELECT 
  subject_id,
  drug,
...
---药物筛查，每种药物使用总人数,注意类不平衡问题，最好不要小于33%
WITH drugs2 AS(
SELECT 
  drug,
  COUNT(*) AS numbers
...
SELECT 
    ...
    alcoholica.has_alcohol_abuse_history,  -- 修正了列名拼写错误
...
SELECT 
    hebing.subject_id,
    MIN(gcs.gcs) AS gcs_score  -- 使用MIN获取最低GCS评分
FROM hebing
...
SELECT 
    hebing.subject_id,
    MAX(apsiii.apsiii) AS apsiii_score  -- 使用MAX获取最高APSIII评分
FROM hebing
...
-- 最终合并
DROP TABLE IF EXISTS final_result;  -- 使用更有意义的表名
...
-- 呼吸机筛查
DROP TABLE IF EXISTS ventilationeded;  -- 使用更有意义的表名
...
DROP TABLE IF EXISTS hfzj;  -- 使用更有意义的表名
...
DROP TABLE IF EXISTS hfzj1;  -- 使用更有意义的表名
...
DROP TABLE IF EXISTS hfzj2;  -- 使用更有意义的表名
...
DROP TABLE IF EXISTS hfzjbun;  -- 使用更有意义的表名
...
DROP TABLE IF EXISTS hfzjmap;  -- 使用更有意义的表名
...
map_window AS (
  ...
  WHERE ce.itemid IN (456, 220045)  -- MAP（有创、无创都取，优先有创）
...
DROP TABLE IF EXISTS hfzjrz;  -- 使用更有意义的表名
...
DROP TABLE IF EXISTS hfzjjs;  -- 使用更有意义的表名
...
steroid_flag AS (
  ...
  WHERE 
    drugs.starttime <= icu.intime  -- 只统计ICU入科前已开具
    AND (
